...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        out_all = (
            minitorch.nn.max(out1, 2)
            + minitorch.nn.max(out2, 2)
            + minitorch.nn.max(out3, 2)
        )
        out_all = out_all.view(embeddings.shape[0], self.feature_map_size)
        l = self.linear.forward(out_all)
        if self.training:
