...
def _mm_practice(out: Storage, a: Storage, b: Storage, size: int) -> None:
    ...
    * Only write to global memory once per kernel.
    ...
    # Data moved to shared memory, and read only once.
    a_shared[x, y] = a[size * x + y]
    b_shared[x, y] = b[size * x + y]
    cuda.syncthreads()

    # calculate: out[i, j] += a[i, k] * b[k, j]
    record = 0.0
    for i in range(size):
        record += a_shared[x, i] * b_shared[i, y]
    out[size * x + y] = record
    # END Task 3.3
...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    # !!!!!!!!!!!!!
    record = 0.0
    for id in range(0, a_shape[2], BLOCK_DIM):
        # Copy into shared memory for a matrix.
        k = id + pj
        if i < a_shape[1] and k < a_shape[2]:
            a = a_batch_stride * batch + a_strides[1] * i + a_strides[2] * k
            # Record into share.
            a_shared[pi, pj] = a_storage[a]

        # Copy into shared memory for b matrix
        k = id + pi
        if j < b_shape[2] and k < b_shape[1]:
            b = b_batch_stride * batch + b_strides[1] * k + b_strides[2] * j
            # Record into share.
            b_shared[pi, pj] = b_storage[b]
        cuda.syncthreads()

        # Compute the dot produce for position c[i, j]
        for k in range(BLOCK_DIM):
            if (id + k) < a_shape[2]:
                record += a_shared[pi, k] * b_shared[k, pj]

    # Fills in `out`
    if i < out_shape[1] and j < out_shape[2]:
        # Get pos of out.
        o = out_strides[0] * batch + out_strides[1] * i + out_strides[2] * j
        out[o] = record
