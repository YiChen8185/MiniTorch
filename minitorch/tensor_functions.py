...
# from .tensor_ops import SimpleBackend, TensorBackend, tensor_map, tensor_zip
...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # TensorData([1])
        (t2,) = ctx.saved_tensors
        return grad_output.f.mul_zip(
            grad_output,
            grad_output.f.mul_zip(
                t2, grad_output.f.add_zip(tensor([1]), grad_output.f.neg_map(t2))
            ),
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        # order_array = order.to_numpy().astype(int).tolist()
        # ctx.save_for_backward(order_array)
        # return a._new(a._tensor.permute(*order_array))
